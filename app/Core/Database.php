...
final class Database
{
    ...
    public static function init(array $config): void
    {
        ...
        self::$driver = ($config['driver'] ?? 'mysql') === 'pgsql' ? 'pgsql' : 'mysql';
    ...
    private static function execSql(string $sql): void
    {
        ...
        foreach ($chunks as $chunk) {
            ...
            if (self::$driver === 'pgsql') {
                $chunk = MysqlToPgsql::convert($chunk);
                if ($chunk === '') {
                    continue;
                }
            }
    ...
    /** @param array{name: string, host: string} $config */
    private static function pgsqlDbName(array $config): string
    {
        $name = (string) $config['name'];
        $endpointId = self::neonEndpointId((string) $config['host']);
        if ($endpointId === null) {
            return $name;
        }

        return $name . ' options=endpoint=' . $endpointId;
    }

    private static function neonEndpointId(string $host): ?string
    {
        if ($host === '' || (!str_contains($host, 'neon.tech') && !str_contains($host, 'neon.database'))) {
            return null;
        }

        $label = explode('.', $host)[0] ?? '';
        if (!str_starts_with($label, 'ep-')) {
            return null;
        }
